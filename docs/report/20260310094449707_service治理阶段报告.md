...
- `protocol_adapter/`：根入口已切换为目录 `mod.rs`，请求映射、响应转换、测试入口都直接挂在子域目录下。
...
- `gateway/observability/http_bridge/mod.rs` 与 `gateway/protocol_adapter/mod.rs` 虽已目录化，但仍承担较多根入口转发职责，后续可继续做更清晰的 facade 分层。
