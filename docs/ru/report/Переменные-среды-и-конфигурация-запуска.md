...
- `CODEXMANAGER_CODEX_IMAGE_GENERATION_AUTO_INJECT_TOOL`: автоматически добавляет tool `image_generation` в обычные `/v1/responses` запросы. Значение по умолчанию `0`; явно переданные клиентом tools всё равно проксируются.
