...
### Added
- Добавлена совместимость генерации изображений Codex: официальный tool `image_generation` передаётся дальше, а endpoints `/v1/images/generations` и `/v1/images/edits` доступны в совместимом режиме; модель image tool по умолчанию — `gpt-image-2`.
