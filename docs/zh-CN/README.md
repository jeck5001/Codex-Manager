...
- 主分支最近新增独立“关于作者”页面：导航入口位于“设置”下方，集中展示作者微信二维码、TG 群聊和赞助码。
- 桌面端“加入 TG 群聊”现在会直接调用系统浏览器打开，修复应用内点击无响应的问题。
- “今日统计”已改为按前端浏览器本地自然日传递 `dayStartTs` / `dayEndTs`，跨时区时不再跟随服务端时区偏移。
