...
- 현재 최신 릴리스: `v0.2.0` (2026-04-12 배포).
- 주요 페이지 전환은 이제 백그라운드 keep-alive 캐시와 전체 영역 loading overlay를 사용하여 desktop, web, Docker 환경에서 재방문 체감이 더 안정적입니다.
- Codex 네이티브 게이트웨이 경로는 기본적으로 공식 클라이언트에 가까운 투명 passthrough 형태로 정리되었고, Claude Code와 Gemini CLI는 계속 호환 어댑터를 사용합니다.
- `service_tier` 전달도 공식 Codex 동작에 맞췄습니다. upstream `/responses` 요청에서는 `fast`가 `priority`로 전송되고 `/responses/compact`에서는 해당 필드가 제거됩니다.
- 이번 릴리스에는 Cloudflare challenge, 호환 포워딩, native Responses flow를 겨냥한 회귀 테스트도 추가되었습니다.
