...
- 메인 브랜치에는 이제 설정 아래의 독립적인 `About the Author` 페이지가 포함되며, 후원 QR 코드, 작성자 WeChat QR, Telegram 그룹 입구를 한곳에 모아 둡니다.
- desktop 앱의 Telegram 그룹 이동은 이제 시스템 브라우저를 직접 호출하므로, 앱 안에서 클릭해도 반응이 없던 문제가 해결되었습니다.
- 오늘 통계는 이제 `dayStartTs` / `dayEndTs` 로 브라우저의 로컬 자연일 범위를 전달하므로, 다른 시간대에서도 service 호스트 시간대에 끌려가지 않습니다.
