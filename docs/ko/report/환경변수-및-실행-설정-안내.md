...
- `CODEXMANAGER_CODEX_IMAGE_GENERATION_AUTO_INJECT_TOOL`: 일반 `/v1/responses` 요청에 `image_generation` tool을 자동 주입할지 여부. 기본값 `0`; 클라이언트가 명시적으로 보낸 tool은 이 값과 관계없이 전달됩니다.
