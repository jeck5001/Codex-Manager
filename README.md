...
## 最近变更
  - 当前最新版本：`v0.1.19`（2026-04-08，release）
  - 聚合 API 已补齐多认证类型与自定义 `action` 路由，第三方最小转发上游现在可以按认证方式和动作更稳定地命中目标接口。
  - 网关已收紧 Responses 路线的 `service_tier` 透传策略，不再把不受支持的值继续发往上游，减少上游因参数不兼容导致的拒绝。
  - 国际化继续补齐：新增语言持久化、拆分消息目录，并完成仪表盘、模态窗、侧边栏和用量标签等剩余文案覆盖。
  - 文档体系也同步整理到当前主线：多语言文档目录、根入口页与 `README` / `CHANGELOG` 的版本说明已统一到 `0.1.19`。

### 近期提交摘要
- `08ef4f9`：支持聚合 API 多认证类型与自定义 `action`。
- `fed557e`：新增语言持久化并拆分消息目录。
- `7803c58`：移除不受支持的 `service_tier` 上游转发。
- `6b721ff`：发布版本统一提升到 `0.1.19`。
