...
- 新增“聚合 API”管理页：支持按 `Codex / Claude` 创建最小转发上游，支持测试连通性、供应商名称、URL、密钥和创建时间展示。
- 平台密钥轮转现在支持 `账号轮转 / 聚合 API 轮转` 两种策略；聚合 API 轮转会按协议直接透传上游请求，账号轮转逻辑保持不变。
...
- 聚合 API：创建、编辑、测试连通性、供应商名称、按 Codex / Claude 分类展示
