...
## 最近变更
  ...
  - 这一版主要补上了 `/v1/responses` 的 WebSocket 请求支持，网关现在可以按传输类型走完整代理链路，并把请求类型、头部归一化、日志记录与运行时行为统一收口。
  - 账号管理页和用量弹窗现在会在刷新后统一展示所有已返回的额度窗口，不只看标准 5 小时 / 7 天额度，也会把 Code Review、Spark 等附加额度一起显示出来，并带上各自的剩余额度与重置时间。
  - `service_tier` 的日志口径也一起补齐：HTTP / WS 请求日志只在客户端显式携带 `service_tier` 时记录 `fast`，不再把平台 Key 默认服务等级误记成请求显式开启。
