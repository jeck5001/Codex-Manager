...
fn gateway_unauthorized_uses_session_cookie_fallback_without_refresh_token() {
    ...
    assert!(
        second.headers.get("cookie").is_some_and(
            |value| value.contains("__Secure-next-auth.session-token=session-gateway-refresh")
        ),
...
#[test]
fn gateway_at_only_account_uses_access_token_directly() {
    let _lock = lock_env();
    let dir = new_test_dir("codexmanager-gateway-openai-at-only-direct");
    let db_path: PathBuf = dir.join("codexmanager.db");
    ...
    let upstream_response = serde_json::json!({
        "id": "resp_at_only_direct",
        "model": "gpt-5.3-codex",
        "output": [{
            "type": "message",
            "role": "assistant",
            "content": [{ "type": "output_text", "text": "ok from access token only" }]
        }],
        "usage": { "input_tokens": 4, "output_tokens": 3, "total_tokens": 7 }
    });
    let upstream_response =
    ...
    storage
        .insert_account(&Account {
            id: "acc_at_only_direct".to_string(),
            label: "at-only-direct".to_string(),
            issuer: "https://auth.openai.com".to_string(),
            chatgpt_account_id: Some("chatgpt_at_only_direct".to_string()),
            workspace_id: None,
            group_name: None,
            sort: 0,
            status: "active".to_string(),
    ...
    storage
        .insert_token(&Token {
            account_id: "acc_at_only_direct".to_string(),
            id_token: String::new(),
            access_token: "access_token_at_only_direct".to_string(),
            refresh_token: String::new(),
            api_key_access_token: None,
            last_refresh: now,
        })
    ...
    let platform_key = "pk_openai_at_only_direct";
    storage
        .insert_api_key(&ApiKey {
            id: "gk_openai_at_only_direct".to_string(),
            name: Some("openai-at-only-direct".to_string()),
            model_slug: Some("gpt-5.3-codex".to_string()),
            reasoning_effort: None,
            client_type: "codex".to_string(),
            protocol_type: "openai_compat".to_string(),
    ...
    assert_eq!(
        captured.headers.get("authorization").map(String::as_str),
        Some("Bearer access_token_at_only_direct")
    );
}
...
fn gateway_at_only_stateless_retry_fails_over_to_next_candidate() {
    ...
    let second_response = serde_json::json!({
        "id": "resp_after_at_only_failover",
        "model": "gpt-5.3-codex",
        "output": [{
            "type": "message",
            "role": "assistant",
            "content": [{ "type": "output_text", "text": "ok after at-only failover" }]
        }],
        "usage": { "input_tokens": 4, "output_tokens": 3, "total_tokens": 7 }
    });
    let body_401 = serde_json::to_string(&first_response).expect("serialize first response");
    ...
    storage
        .insert_account(&Account {
            id: "acc_at_only_failover_bad".to_string(),
            label: "at-only-failover-bad".to_string(),
            issuer: "https://auth.openai.com".to_string(),
            chatgpt_account_id: Some("chatgpt_at_only_failover_bad".to_string()),
            workspace_id: None,
            group_name: None,
            sort: 0,
            status: "active".to_string(),
    ...
    storage
        .insert_token(&Token {
            account_id: "acc_at_only_failover_bad".to_string(),
            id_token: String::new(),
            access_token: "access_token_at_only_failover_bad".to_string(),
            refresh_token: String::new(),
            api_key_access_token: None,
            last_refresh: now,
        })
    ...
    storage
        .insert_account(&Account {
            id: "acc_at_only_failover_good".to_string(),
            label: "at-only-failover-good".to_string(),
            issuer: "https://auth.openai.com".to_string(),
            chatgpt_account_id: Some("chatgpt_at_only_failover_good".to_string()),
            workspace_id: None,
            group_name: None,
            sort: 1,
            status: "active".to_string(),
    ...
    storage
        .insert_token(&Token {
            account_id: "acc_at_only_failover_good".to_string(),
            id_token: String::new(),
            access_token: "access_token_at_only_failover_good".to_string(),
            refresh_token: String::new(),
            api_key_access_token: None,
            last_refresh: now + 1,
        })
    ...
    let platform_key = "pk_openai_at_only_failover";
    storage
        .insert_api_key(&ApiKey {
            id: "gk_openai_at_only_failover".to_string(),
            name: Some("openai-at-only-failover".to_string()),
            model_slug: Some("gpt-5.3-codex".to_string()),
            reasoning_effort: None,
            client_type: "codex".to_string(),
            protocol_type: "openai_compat".to_string(),
...
fn gateway_invalid_session_cookie_fails_over_to_next_candidate() {
    ...
    let body_200 = serde_json::to_string(&second_response).expect("serialize second response");
    let (upstream_addr, upstream_rx, upstream_join) =
        start_mock_upstream_sequence(vec![(401, body_401), (401, String::new()), (200, body_200)]);
...
#[test]
fn gateway_token_invalidated_marks_account_unavailable_and_fails_over() {
    let _lock = lock_env();
    let dir = new_test_dir("codexmanager-gateway-token-invalidated-failover");
    let db_path: PathBuf = dir.join("codexmanager.db");
    ...
    let first_body = serde_json::to_string(&serde_json::json!({
        "error": {
    ...
    let second_body = serde_json::to_string(&serde_json::json!({
        "id": "resp_after_token_invalidated_failover",
        "model": "gpt-5.3-codex",
        "output": [{
            "type": "message",
            "role": "assistant",
            "content": [{ "type": "output_text", "text": "ok after token invalidated failover" }]
        }],
        "usage": { "input_tokens": 5, "output_tokens": 4, "total_tokens": 9 }
    }))
    .expect("serialize second response");
    let (upstream_addr, rx, upstream_join) = start_mock_upstream_sequence_with_headers(vec![
        (
            401,
            first_body,
            vec![
    ...
    storage
        .insert_account(&Account {
            id: "acc_token_invalidated_bad".to_string(),
            label: "token-invalidated-bad".to_string(),
            issuer: "https://auth.openai.com".to_string(),
            chatgpt_account_id: Some("chatgpt_token_invalidated_bad".to_string()),
            workspace_id: None,
            group_name: None,
            sort: 0,
            status: "active".to_string(),
    ...
    storage
        .insert_token(&Token {
            account_id: "acc_token_invalidated_bad".to_string(),
            id_token: String::new(),
            access_token: "access_token_token_invalidated_bad".to_string(),
            refresh_token: String::new(),
            api_key_access_token: None,
            last_refresh: now,
        })
    ...
    storage
        .insert_account(&Account {
            id: "acc_token_invalidated_good".to_string(),
            label: "token-invalidated-good".to_string(),
            issuer: "https://auth.openai.com".to_string(),
            chatgpt_account_id: Some("chatgpt_token_invalidated_good".to_string()),
            workspace_id: None,
            group_name: None,
            sort: 1,
            status: "active".to_string(),
    ...
    storage
        .insert_token(&Token {
            account_id: "acc_token_invalidated_good".to_string(),
            id_token: String::new(),
            access_token: "access_token_token_invalidated_good".to_string(),
            refresh_token: String::new(),
            api_key_access_token: None,
            last_refresh: now + 1,
        })
    ...
    let platform_key = "pk_openai_token_invalidated_failover";
    storage
        .insert_api_key(&ApiKey {
            id: "gk_openai_token_invalidated_failover".to_string(),
            name: Some("openai-token-invalidated-failover".to_string()),
            model_slug: Some("gpt-5.3-codex".to_string()),
            reasoning_effort: None,
            client_type: "codex".to_string(),
            protocol_type: "openai_compat".to_string(),
    ...
    assert_eq!(
        first.headers.get("authorization").map(String::as_str),
        Some("Bearer access_token_token_invalidated_bad")
    );
    assert_eq!(second.path, "/chatgpt.com/backend-api/codex/responses");
    assert_eq!(
        second.headers.get("authorization").map(String::as_str),
        Some("Bearer access_token_token_invalidated_good")
    );

    let bad_account = storage
        .find_account_by_id("acc_token_invalidated_bad")
        .expect("find first account")
        .expect("first account exists");
    assert_eq!(bad_account.status, "unavailable");
}
