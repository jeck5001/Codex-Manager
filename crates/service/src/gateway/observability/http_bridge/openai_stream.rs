...
use super::collect_response_output_text;

#[derive(Debug, Clone, Default)]
pub(super) struct OpenAIStreamMeta {
    pub(super) response_id: Option<String>,
    pub(super) model: Option<String>,
    pub(super) created: Option<i64>,
}

pub(super) fn update_openai_stream_meta(meta: &mut OpenAIStreamMeta, value: &Value) {
...
pub(super) fn apply_openai_stream_meta_defaults(mapped: &mut Value, meta: &OpenAIStreamMeta) {
...
pub(super) fn extract_openai_completed_output_text(value: &Value) -> Option<String> {
...
pub(super) fn map_chunk_has_chat_text(mapped: &Value) -> bool {
    mapped
        .get("choices")
...
pub(super) fn map_chunk_has_completion_text(mapped: &Value) -> bool {
    mapped
        .get("choices")
        .and_then(Value::as_array)
        .is_some_and(|choices| {
...
pub(super) fn should_skip_chat_live_text_event(event_type: &str, value: &Value) -> bool {
    match event_type {
...
pub(super) fn should_skip_completion_live_text_event(event_type: &str, value: &Value) -> bool {
    match event_type {
        "response.output_text.done"
        | "response.content_part.added"
        | "response.content_part.delta"
...
pub(super) fn normalize_chat_chunk_delta_role(mapped: &mut Value, role_emitted: &mut bool) {
...
pub(super) fn build_chat_fallback_content_chunk(meta: &OpenAIStreamMeta, content: &str) -> Value {
    json!({
...
pub(super) fn build_completion_fallback_text_chunk(meta: &OpenAIStreamMeta, text: &str) -> Value {
    json!({
        "id": meta.response_id.clone().unwrap_or_default(),
...
pub(super) fn synthesize_chat_completion_sse_from_json(value: &Value) -> Vec<u8> {
...
pub(super) fn synthesize_completions_sse_from_json(value: &Value) -> Vec<u8> {
    let Some(root) = value.as_object() else {
        return b"data: [DONE]\n\n".to_vec();
    };
    if root.contains_key("error") {
