    ...
    F: FnMut(Option<&str>, u16, Option<&str>),
{
    if matches!(status.as_u16(), 429 | 500..=599) {
        ...
        super::mark_account_cooldown_for_status(account_id, status.as_u16());
    }
    if status.is_success() {
        super::clear_account_cooldown(account_id);
        log_gateway_result(Some(url), status.as_u16(), None);
        return UpstreamOutcomeDecision::RespondUpstream;
    }

    let is_challenge = super::is_upstream_challenge_response(status.as_u16(), upstream_content_type);
    if is_challenge {
        super::mark_account_cooldown(account_id, super::CooldownReason::Challenge);
    ...
    let should_failover = super::should_failover_after_refresh(storage, account_id, refresh_result);
    if should_failover {
        super::mark_account_cooldown_for_status(account_id, status.as_u16());
    }
